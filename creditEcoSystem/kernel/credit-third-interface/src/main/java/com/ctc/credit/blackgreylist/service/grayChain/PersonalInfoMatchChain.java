 ...
 */
@Component
public class PersonalInfoMatchChain  extends IAbstractGrayMatchChain {
	@Autowired
	...
	public List<Map<String, Object>> doMatch(HandleRequest requestInfo) {
		...
		List<Map<String,Object>> matchResult = blkgrayListRoleServiceImpl.matchPersonalInfo(requestInfo.getCardNo(), requestInfo.getMobile(), requestInfo.getAddressDetail(), requestInfo.getPhone());
		...
		for(String key : matchInfo.keySet()){
			...
			mm.put("glistName", "疑似欺诈客户灰名单B");
			mm.put("glistRole", "身份证号码||手机号码||住宅地址||住宅电话");
			result.add(mm);
		}
