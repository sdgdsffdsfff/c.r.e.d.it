...
public class CreditHtmlAnalysisService extends AbstractAnalysisService {
	...
	private static Logger logger = Logger
			.getLogger(CreditHtmlAnalysisService.class);
	...
	public ReportHeadInfo getReportHeadInfo() throws Exception {
		try {
			...
			WebElement e = analysisDriver.getElement(this.getDriver(), "报告编号", "查询请求时间",
					"报告时间");
	...
	public QueryPersonInfo getQueryPersonInfo() throws Exception {
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "被查询者姓名", "被查询者证件类型",
				"被查询者证件号码", "查询原因");
		Map<String, String> map = analysisDriver.getKeyValue(e, "被查询者姓名", "被查询者证件类型",
				"被查询者证件号码", "查询原因");
	...
	public PersonalInfo getPersonalInfo() throws Exception {
		WebElement e1 = analysisDriver.getElement(this.getDriver(), "通讯地址", "户籍地址");
		Map<String, String> map = analysisDriver.getKeyValue(e1, "通讯地址", "户籍地址");
		WebElement e2 = analysisDriver.getElement(this.getDriver(), "出生日期", "单位电话", "学位",
				"学历", "住宅电话", "婚姻状况", "手机号码");
		map.putAll(analysisDriver.getKeyValue(e2, "出生日期", "单位电话", "学位", "学历", "住宅电话",
				"婚姻状况", "手机号码"));
	...
	public PersonalSpouseInfo getPersonalSpouseInfo() throws Exception {
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "姓名", "证件号码", "证件类型",
				"工作单位", "联系电话");
		Map<String, String> map = analysisDriver.getKeyValue(e, "姓名", "证件号码", "证件类型",
				"工作单位", "联系电话");
	...
	public List<PersonalResidentialInfo> getPersonalResidentialInfos()
		...
		WebElement element = analysisDriver.getElement(this.getDriver(), "居住地址", "居住状况",
				"编号", "信息更新日期");
		List<Map<String, String>> addrInfos = analysisDriver.getListData(null, element,
	...
	public List<PersonalCareerInfo> getPersonalCareerInfos() throws Exception {
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "单位地址", "编号", "工作单位");
		WebElement e2 = analysisDriver.getElement(this.getDriver(), "行业", "职业", "职称",
				"进入本单位年份", "信息更新日期");

		List<Map<String, String>> infos = analysisDriver.getListData(e, e2, new String[] {
	...
	public SummaryCreditInfo getSummaryCreditInfo() throws Exception {
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "发卡月份", "住房贷款");
		Map<String, String> map = analysisDriver.getKeyValue(e, "本人声明数目", "贷记卡账户数",
	...
	public BankCreditInfo getBankCreditInfo() throws Exception {
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "评分月份", "中征信评分");

		Map<String, String> map = analysisDriver.getKeyValue(e, "中征信评分", "评分月份");
	...
	public SummaryOverdueAndDefaultInfo getSummaryOverdueAndDefaultInfo()
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "呆账信息汇总", "资产处置信息汇总",
				"保证人代偿信息汇总");

		Map<String, String> map = analysisDriver.getKeyValueSpecial(e, new String[] {
	...
	public SummaryOverdueInfo getSummaryOverdueInfo() throws Exception {
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "贷记卡逾期", "贷款逾期",
				"准贷记卡60天以上透支");
	...
	public SummaryOutstandingInfo getSummaryOutstandingInfo() throws Exception {
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "合同总额", "贷款法人机构数",
				"贷款机构数", "余额");
		Map<String, String> map = analysisDriver.getKeyValue(e, "余额", "合同总额", "笔数", "笔数",
	...
	public SummaryNotCloseCreditInfo getSummaryNotCloseCreditInfo()
		...
		WebElement e = analysisDriver
				.getElement(this.getDriver(), "已用额度", "发卡机构数", "账户数",
						"发卡法人机构数", "授信总额", "单家行最高授信额", "最近6个月平均使用额度",
						"单家行最低授信额");
		Map<String, String> map = analysisDriver.getKeyValue(e, "已用额度", "发卡机构数", "账户数",
	...
	public SummaryQuasiCreditInfo getSummaryQuasiCreditInfo() throws Exception {
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "透支余额", "账户数",
				"发卡机构数", "发卡法人机构数", "授信总额", "单家行最高授信额", "最近6个月平均透支余额",
				"单家行最低授信额");
		Map<String, String> map = analysisDriver.getKeyValue(e, "透支余额", "账户数", "发卡机构数",
	...
	public SummaryGuaranteeInfo getSummaryGuaranteeInfo() throws Exception {
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "担保金额", "担保笔数",
				"担保本金余额");
		Map<String, String> map = analysisDriver.getKeyValue(e, "担保金额", "担保笔数", "担保本金余额");
	...
	public List<AssertDisposeDetailInfo> getAssertDisposeDetailInfos()
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "资产管理公司", "债务接收日期",
				"最近一次还款日期");
		List<Map<String, String>> listData = analysisDriver.getListData(e, "资产管理公司",
	...
	public List<GuarantorCompensationDetailInfo> getGuarantorCompensationDetailInfos()
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "代偿机构", "最近一次代偿日期",
				"累计代偿金额");
		List<Map<String, String>> listData = analysisDriver.getListData(e, "代偿机构",
	...
	public List<LoanInfo> getLoanInfos() throws Exception {
		...
		// List<WebElement> elements = analysisDriver.getElements(e, new String[] { "贷记卡"
		// },
		// finder);
		//
		...
		List<WebElement> titles = this.getTitles("贷款");
		for (int i = 0; i < titles.size(); i++) {
			WebElement title = titles.get(i);
			String totalDescription = titles.get(i).getText()
					.replaceAll("\\s", "");
			if (!title.getText().contains("贷记卡")) {
				// System.out.println(title.getText().replaceAll("\\s", ""));
				LoanInfo loanInfo = new LoanInfo();
	...
	private void setLoanInfo(LoanInfo loanInfo, WebElement title)
		...
		if (txt.contains("贷款种类") && txt.contains("业务号")
			...
			Map<String, String> map = analysisDriver.getKeyValue(title, "贷款种类", "业务号",
					"贷款机构名称", "担保方式", "币种");
		...
		String e = analysisDriver.getNextElement(title).getText().replaceAll("\\s", "");
		...
		for (int i = 0; i < trs.size(); i++) {
			...
			if (tr_value.contains("五级分类") && tr_value.contains("应还款日")) {
				Map<String, String> map = analysisDriver.getKeyValue(ele, "五级分类", "本金余额",
						"剩余还款期数", "本月应还款", "应还款日");
			...
			if (tr_value.contains("未还本金") && tr_value.contains("当前逾期")) {
				Map<String, String> map = analysisDriver.getKeyValue(ele, "当前逾期期数",
			...
			if (tr_value.contains("贷款机构说明") && tr_value.contains("添加日期")) {
				Map<String, String> map = analysisDriver.getKeyValue(ele, "贷款机构说明",
						"添加日期");
			...
			if (tr_value.contains("本人声明") && tr_value.contains("添加日期")) {
				Map<String, String> map = analysisDriver.getKeyValue(ele, "本人声明", "添加日期");
			...
			if (tr_value.contains("异议标注") && tr_value.contains("添加日期")) {
				Map<String, String> map = analysisDriver.getKeyValue(ele, "异议标注", "添加日期");
			...
			if (tr_value.contains("发生日期") && tr_value.contains("发生金额")
				...
				List<SpecialTradeType> specialTradeTypes = new ArrayList<SpecialTradeType>();

				for (int index = 1; index < 100; index++) {
					WebElement valueObjs = ele.findElement(By
							.xpath("./following-sibling::tr[" + index + "]"));

					List<WebElement> keyObjs = ele.findElements(By
							.xpath("./child::*"));
					List<WebElement> childValueObjs = valueObjs.findElements(By
							.xpath("./child::*"));
					if (childValueObjs.size() == keyObjs.size()) {

						Map<String, String> map = new HashMap<String, String>();

						for (int j = 0; j < keyObjs.size(); j++) {
							map.put(keyObjs.get(j).getText()
									.replaceAll("\\s", ""),
									childValueObjs.get(j).getText()
											.replaceAll("\\s", ""));
						}

						SpecialTradeType specialTradeType = new SpecialTradeType();

						specialTradeType.setSpecialTradeType(map.get("特殊交易类型"));
						specialTradeType.setOccurAmount(map.get("发生金额"));
						specialTradeType.setOccurDate(map.get("发生日期"));
						specialTradeType.setOverdueRemark(map.get("明细记录"));
						specialTradeType.setChangedMonthCount(map.get("变更月数"));
						specialTradeTypes.add(specialTradeType);
					} else {
						break;
					}
				}
				loanInfo.setSpecialTradeTypes(specialTradeTypes);
			}

			if (tr_value.contains("的还款记录")) {
				WebElement value = ele.findElement(By
						.xpath("./following::tr[1]"));
				loanInfo.setRepaymentRecord_24(value.getText().replaceAll(
						"\\s", ""));
				loanInfo.setRepaymentRecord_24Title(tr_value);
			}
			...
			if (tr_value.contains("状态") && tr_value.contains("状态截止日")) {
				...
				try {
					...
					if (size <= 4) {
						...
						map.putAll(analysisDriver.getKeyValue(trs.get(i), "状态", "状态截止日"));
	...
	public List<WebElement> getTitles(String name) {
		...
		List<WebElement> txts = driver.findElements(By.xpath(xpath));

		WebElement bigTE = null;

		for (int i = txts.size() - 1; i >= 0; i--) {
			String bigTitle = txts.get(i).getText().replaceAll("\\s", "");
			if ((bigTitle.contains("）" + name) || bigTitle.contains(")" + name))
					&& !bigTitle.contains(",") && !bigTitle.contains("，")) {
//				System.out.println("bigTitle" + bigTitle);
				bigTE = txts.get(i);
				break;
			}
		}
		...
		if (bigTE != null) {
			List<WebElement> findElements = bigTE
					.findElements(By
							.xpath("./following::tr | ./following::p | ./following::h4"));

			// System.out.println(findElements.size());
			for (WebElement e : findElements) {
				// System.out.println(e.getText().replaceAll("\\s", ""));
				if (name.equals("贷款")) {
					if (e.getText().replaceAll("\\s", "").contains("贷记卡")) {
						break;
					}
				}
				if (name.equals("贷记卡")) {
					if (e.getText().replaceAll("\\s", "").contains("准贷记卡")) {
						break;
					}
				}

				String xpathExpression = ".//table";
				List<WebElement> els = e
						.findElements(By.xpath(xpathExpression));
				if (els != null && els.size() > 0) {
					continue;
				} else {
					if (name.equals("贷款")) {
						String txt = e.getText().replaceAll("\\s", "");
						if (txt.contains("贷款") && txt.contains("业务号")
								&& !txt.contains("贷款机构名称")) {
							map.put(txt, e);
						} else if (txt.contains("贷款") && txt.contains("业务号")
								&& txt.contains("贷款机构名称")) {
							titles.add(e);
						}
					} else {
						...
						String txt = e.getText().replaceAll("\\s", "");
						if (txt.contains("授信额度") && txt.contains("业务号")
								&& !txt.contains("发卡机构")) {
							map.put(txt, e);
						} else if (txt.contains("授信额度") && txt.contains("业务号")
								&& txt.contains("发卡机构")) {
							titles.add(e);
						}
					}
...
//			System.out.println(v + "----");
	...
	public List<CreditCardInfo> getCreditCardInfos() throws Exception {
		...
		this.putCreditCardInfo(creditCardInfos, this.getTitles("贷记卡"));

		// String xpath =
		// "//*[contains(text(),'）贷记卡')]/ancestor::*/following-sibling::*";
		// // 获取所有的贷记卡信息
		// List<WebElement> txts = driver.findElements(By.xpath(xpath));//
		// 个人信用报告（银行专业版）样本.htm
		//
		...
		// txts = driver
		// .findElements(By
		// .xpath("//*[contains(text(),'授信额度')]/following::tr | //*[contains(text(),'授信额度')]/ancestor::tr[1] | //*[contains(text(),'授信额度')]/following::tr/following::b | //*[contains(text(),'授信额度')]/following::tr/following::h4 "));//
		// 陈明.htm
	...
	private void putCreditCardInfo(List<CreditCardInfo> creditCardInfos,
		...
		for (int i = 0; i < txts.size(); i++) {
			...
			if (titleStr.length() > 30 && titleStr.contains("发放的")
					&& titleStr.contains("贷记卡") && titleStr.startsWith("^\\d$")) {
				...
				if (titleStr.contains("业务号") && titleStr.contains("授信额度")) {
					CreditCardInfo cardInfo = new CreditCardInfo();
					cardInfo.setTotalDescription(titleStr);
					this.setCreditCardInfo(cardInfo, title);
					creditCardInfos.add(cardInfo);
	...
	private void setCreditCardInfo(CreditCardInfo creditCardInfo,
		...
		if (txt.contains("发卡机构") && txt.contains("业务号") && txt.contains("币种")
				&& txt.contains("发卡日期")) {
			Map<String, String> map = analysisDriver.getKeyValue(title, "发卡机构", "业务号",
					"发卡日期");
		...
		for (int i = 0; i < trs.size(); i++) {
			...
			if (tr_value.contains("共享额度") && tr_value.contains("已用额度")) {
				Map<String, String> map = analysisDriver.getKeyValue(ele, "共享额度", "已用额度",
						"最大使用额度");
			...
			if (tr_value.contains("账单日") && tr_value.contains("本月实还款")
				...
				Map<String, String> map = analysisDriver.getKeyValue(ele, "账单日", "本月实还款",
						"当前逾期期数");
				Set<String> keySet = map.keySet();
				Iterator<String> it = keySet.iterator();
				while (it.hasNext()) {
			...
			if (tr_value.contains("发生日期") && tr_value.contains("特殊交易类型")) {
				...
				creditCardInfo.setSpecialTradeTypes(specialTradeTypes);
			}
			if (tr_value.contains("发卡机构说明") && tr_value.contains("添加日期")) {
				Map<String, String> map = analysisDriver.getKeyValue(ele, "发卡机构说明",
						"添加日期");
			...
			if (tr_value.contains("的还款记录")) {
				WebElement value = ele.findElement(By
						.xpath("./following::tr[1]"));
				creditCardInfo.setRepaymentRecord_24(value.getText()
						.replaceAll("\\s", ""));
				creditCardInfo.setRepaymentRecord_24Title(tr_value);
			}
			if (tr_value.contains("状态") && tr_value.contains("状态截止日")) {
				Map<String, String> map = new HashMap<String, String>();
				try {
	...
	public List<QuasiCreditInfo> getQuasiCreditInfos() throws Exception {
		...
		// String xpath =
		// "//*[contains(text(),'）准贷记卡')]/ancestor::*/following-sibling::*";
		// // 获取所有的贷记卡信息
		// List<WebElement> txts = driver.findElements(By.xpath(xpath));//
		// 个人信用报告（银行专业版）样本.htm
		//
		...
		this.putQuasiCreditInfo(quasiCreditInfos, this.getTitles("准贷记卡"));
	...
	private void putQuasiCreditInfo(List<QuasiCreditInfo> quasiCreditInfos,
		...
		for (int i = 0; i < txts.size(); i++) {
			String titleStr = txts.get(i).getText().replaceAll("\\s", "");
			WebElement title = txts.get(i);
			if (titleStr.contains("欠税记录") || titleStr.contains("资产处置")) {
				break;
			}
	...
	private void QuasiCreditInfo(QuasiCreditInfo quasiCreditInfo,
		...
		for (int i = 0; i < trs.size(); i++) {
			...
			if (tr_value.contains("共享额度") && tr_value.contains("透支余额")
				...
				Map<String, String> map = analysisDriver.getKeyValue(ele, "共享额度", "透支余额",
						"最大透支余额");
			...
			if (tr_value.contains("发生日期") && tr_value.contains("特殊交易类型")) {
				...
				quasiCreditInfo.setSpecialTradeTypes(specialTradeTypes);
			}
			if (tr_value.contains("发卡机构说明") && tr_value.contains("添加日期")) {
				Map<String, String> map = analysisDriver.getKeyValue(ele, "发卡机构说明",
						"添加日期");
			...
			if (tr_value.contains("的还款记录")) {
				...
				quasiCreditInfo.setRepaymentRecord_24(value.getText()
						.replaceAll("\\s", ""));
				quasiCreditInfo.setRepaymentRecord_24Title(tr_value);
			}
			if (tr_value.contains("状态") && tr_value.contains("状态截止日")) {
				Map<String, String> map = new HashMap<String, String>();
				try {
	...
	public List<GuaranteeDetailInfo> getGuaranteeDetailInfos() throws Exception {
		...
		List<Map<String, String>> listData = analysisDriver.getListData(e, "担保贷款到期日期",
	...
	public List<TaxesInfo> getTaxesInfos() throws Exception {
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "欠税总额", "欠税统计日期",
				"主管税务机关");
		List<Map<String, String>> listData = analysisDriver.getListData(e, "欠税总额",
				"欠税统计日期", "主管税务机关");
	...
	public List<CivilJudgmentInfo> getCivilJudgmentInfos() throws Exception {
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "立案法院", "立案日期", "案由");
		WebElement e2 = analysisDriver.getElement(this.getDriver(), "诉讼标的", "诉讼标的金额",
				"生效日期");
	...
	public List<EnforcementInfo> getEnforcementInfos() throws Exception {
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "执行法院", "执行案由",
				"立案日期", "结案方式");
		WebElement e2 = analysisDriver.getElement(this.getDriver(), "案件状态", "结案日期",
				"申请执行标的", "申请执行标的价值");
		List<Map<String, String>> listData = analysisDriver.getListData(e, e2,
	...
	public List<AdminPunishmentInfo> getAdminPunishmentInfos() throws Exception {
		...
		List<Map<String, String>> listData = analysisDriver.getListData(e, "处罚机构",
	...
	public HousingFundPayInfo getHousingFundPayInfo() throws Exception {
		...
		WebElement e1 = analysisDriver
				.getElement(this.getDriver(), "参缴日期", "参缴地", "初缴月份");
		Map<String, String> map = analysisDriver.getKeyValue(e1, "参缴日期", "参缴地", "初缴月份");
		WebElement e2 = analysisDriver.getElement(this.getDriver(), "缴费单位", "信息更新日期");
	...
	public PensionPayInfo getPensionPayInfo() throws Exception {
		...
		WebElement e1 = analysisDriver.getElement(this.getDriver(), "参保日期", "参保地",
				"参加工作月份");
		Map<String, String> map = analysisDriver.getKeyValue(e1, "参保日期", "参保地", "参加工作月份");
		WebElement e2 = analysisDriver.getElement(this.getDriver(), "缴费单位", "中断或终止缴费原因");
	...
	public PensionReceiveInfo getPensionReceiveInfo() throws Exception {
		...
		WebElement e1 = analysisDriver.getElement(this.getDriver(), "离退休类别", "发放地",
				"离退休月份");
		Map<String, String> map = analysisDriver.getKeyValue(e1, "离退休类别", "发放地", "离退休月份");
		WebElement e2 = analysisDriver.getElement(this.getDriver(), "原单位名称", "信息更新日期");
	...
	public List<LowReliefInfo> getLowReliefInfos() throws Exception {
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "人员类别", "所在地", "工作单位");
		List<Map<String, String>> listData = analysisDriver.getListData(e, "人员类别", "所在地",
				"工作单位");
	...
	public List<QualificationInfo> getQualificationInfos() throws Exception {
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "执业资格名称", "吊销日期",
				"获得日期");
		List<Map<String, String>> listData = analysisDriver.getListData(e, "执业资格名称",
				"吊销日期", "获得日期");
		for (Map<String, String> map : listData) {
	...
	public List<AdministrativeRewardInfo> getAdministrativeRewardInfos()
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "奖励机构", "奖励内容",
				"生效日期", "截止日期");
		List<Map<String, String>> listData = analysisDriver.getListData(e, "奖励机构",
				"奖励内容", "生效日期", "截止日期");
	...
	public List<VehicleTradeAndMortgageInfo> getVehicleTradeAndMortgageInfos()
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "车牌号码", "车辆类型",
				"抵押标记", "信息更新日期");
		List<Map<String, String>> listData = analysisDriver.getListData(e, "车牌号码",
				"车辆类型", "抵押标记", "信息更新日期");
		for (Map<String, String> map : listData) {
	...
	public List<TelecomPaymentInfo> getTelecomPaymentInfos() throws Exception {
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "电信运营商", "业务类型",
				"当前欠费金额", "业务开通日期");

		List<Map<String, String>> listData = analysisDriver.getListData(e, new String[] {
	...
	public List<LoanDeclareDetailInfo> getLoanDeclareDetailInfos()
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "声明内容", "添加日期");

		List<Map<String, String>> listData = analysisDriver
	...
	public List<DissentDeclareDetailInfo> getDissentDeclareDetailInfos()
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "标注内容", "添加日期");

		List<Map<String, String>> listData = analysisDriver.getListData(e, "标注内容", "添加日期");
		for (Map<String, String> map : listData) {
	...
	WebElement e = analysisDriver.getElement(this.getDriver(), "贷后管理", "贷款审批");
	Map<String, String> map = null;
	try {
		List<WebElement> tables = e.findElements(By
				.xpath("./descendant::table"));
		if (tables.size() == 3) {
			map = new HashMap<String, String>();

			for (WebElement t : tables) {
				List<WebElement> trs = t.findElements(By.xpath(".//tr"));

				String bigTitle = trs.get(0).getText()
						.replaceAll("\\s", "");

				List<WebElement> smallTitle = trs.get(1).findElements(
						By.xpath("./child::*"));

				List<WebElement> values = trs.get(2).findElements(
						By.xpath("./child::*"));

				for (int i = 0; i < smallTitle.size(); i++) {
					WebElement sTxt = smallTitle.get(i);
					String smallTxt = sTxt.getText().replaceAll("\\s", "");
					...
					// smallTxt+"  :  "+value);
					map.put(bigTitle + smallTxt, value);
				}

			}
		} else {
			map = analysisDriver.getKeyValueSpecial(
					e.findElement(By.xpath("./preceding-sibling::*[1]")),
					new String[] { "个月内的查询机构数", "内的查询次数" }, "贷后管理", "贷款审批");
		}
	} catch (Exception ex) {

		System.out.println("getSummaryQueryRecordInfo异常");
	}
	...
	last1MonthQueryInstitution.setCreditCardApprovalCount(map
			.get("最近1个月内的查询机构数信用卡审批") == null ? map
			.get("最近一个月内的查询机构数信用卡审批") : map.get("最近1个月内的查询机构数信用卡审批"));

	last1MonthQueryInstitution.setLoanApprovalCount(map
			.get("最近1个月内的查询机构数贷款审批") == null ? map.get("最近一个月内的查询机构数贷款审批")
			: map.get("最近1个月内的查询机构数贷款审批"));
	last1MonthQuery
			.setCreditCardApprovalCount(map.get("最近1个月内的查询次数信用卡审批") == null ? map
					.get("最近一个月内的查询次数信用卡审批") : map.get("最近1个月内的查询次数信用卡审批"));
	last1MonthQuery
			.setLoanApprovalCount(map.get("最近1个月内的查询次数贷款审批") == null ? map
					.get("最近一个月内的查询次数贷款审批") : map.get("最近1个月内的查询次数贷款审批"));
	...
	public List<QueryRecordInfo> getQueryRecordInfos() throws Exception {
		...
		WebElement e = analysisDriver.getElement(this.getDriver(), "查询操作员", "查询日期",
				"查询原因");

		List<Map<String, String>> listData = analysisDriver.getListData(e, "查询操作员",
				"查询日期", "查询原因");
		for (Map<String, String> map : listData) {
